...
const InterviewForm: React.FC<{
	...
	const [selectedInterviewers, setSelectedInterviewers] = useState<EmployeeDetails[]>(
		stage.interviewers || [],
	);
	...
	const handleSelect = (value: string | undefined) => {
		if (value) {
			const selected = employeeOptions.find((emp) => `${emp.name} ${emp.surname}` === value);
			if (selected && !selectedInterviewers!.some((emp) => emp.email === selected.email)) {
		...
		<>
			<Row gutter={16}>
				...
				<Col span={12}>
					<FormInputs.Select
						...
						options={interviewTypes.map((item) => {
							return {
								label: t(item.label),
								value: item.value,
							};
			...
			<Form.Item label={t("interviewers")} name={"interviewers"}>
				<Row style={{ paddingBottom: "5px", width: "100%" }}>
					<Select
						mode="multiple"
						placeholder={t("selectInterviewers")}
						tagRender={tagRender}
						...
						value={selectedInterviewers.map((emp) => `${emp.name} ${emp.surname}`)}
					/>
				</Row>
			</Form.Item>
