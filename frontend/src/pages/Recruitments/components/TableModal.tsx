...
const options = selectOption.map((option) => ({ value: option.label, label: option.label }));
...
const TableModal = forwardRef(({ selectedRecord, onAdd, onEdit }: EditModalProps, ref) => {
	...
	function handleSubmit(values: any) {
		const valuesToSend = {
			...values,
			submittedDate: values.submittedDate ? new Date(values.submittedDate) : dayjs(),
		};

		const path = selectedRecord ? `${API}/${selectedRecord._id}` : API;

		sendRequest(
			useHttp[selectedRecord ? "patchRequestHelper" : "postRequestHelper"](path, valuesToSend),
			(responseData: any) => {
		...
		>
			<Flex gap={45}>
				...
				<Flex vertical style={{ width: "100%" }}>
					...
					<FormInputs.DatePicker name="submittedDate" label="Date Submitted" isDisabledDate />
				</Flex>
			</Flex>
			{error && <Typography.Text type="danger">{"SUPER MEGA XHEST PLAK"}</Typography.Text>}
		</Form>
	);
});
