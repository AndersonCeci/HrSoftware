...
export const WelcomeGrid: React.FC<WelcomeGridProps> = ({ initialData }) => {
  ...
  const [tableEmployeeData, setTableEmployeeData] =
    useState<EmployeeDataType>();
...
// console.log(loadedEvents.length, 'eventsss')
                      ...
                      {item.status === "holiday" ? (
                        <Flex className="holiday-card">
                          {holidayData.map((data, index) => {
                            return (
                              <div key={index}>
                                <p style={{ color: "white", marginRight:"10px", justifyContent:"center" }}>
                                  {data.paragraph}
                                </p>
                                <br />
