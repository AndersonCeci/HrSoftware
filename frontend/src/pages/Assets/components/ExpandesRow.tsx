...
const ExpandedRow = ({ record }: { record: AssetDatatype }) => {
  ...
  const { updateInventaryItemHandler, deleteFromInventaryHandler } = useContext(
    AssetInventaryContext,
  );
  const [selectedAsset, setSelectedAsset] = useState<InventaryDataType | null>(
    null,
  );
  ...
  function handleOnAssign(dataToSubmit: {
    ...
    fetchData(
      useHttp.patchRequestHelper(
        `${INVENTARY_API}/assign/${selectedAsset?._id}`,
        dataToSubmit,
      ),
      (response) => {
        updateInventaryItemHandler(response, {
  ...
  function handleDeleteFromInventary(deletedInventary: InventaryDataType) {
    fetchData(
      useHttp.deleteRequestHelper(`${INVENTARY_API}/${deletedInventary._id}`),
      () => {
