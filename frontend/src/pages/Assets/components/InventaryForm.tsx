...
const QuantityForm = forwardRef(
  (
    { selectedAsset, onAddAssetType, onAddQuantity }: QuantityFormProps,
    ref,
  ) => {
    ...
    return (
      <Form
        ...
        autoComplete="off"
        onFinish={(values) =>
          onFinish(values, selectedAsset ? "codes" : "assetName")
        }
      >
        <Form.List
              ...
              validator: async (_, values) => {
                if (!values || values.length < 1) {
                  return Promise.reject(
                    new Error("At least 1 Asset is required"),
                  );
                }
              },
            },
            {
              validator: async (_, values) => {
                ...
                if (values.length !== uniqueValues.size) {
                  ...
                  return Promise.reject(
                    new Error(
                      `Duplicate values found: ${duplicateValues.values().next().value}`,
                    ),
                  );
                }
              },
                ...
                >
                  ...
                  >
                    <Input
                      ...
                      placeholder={
                        selectedAsset
                          ? `${selectedAsset.assetName}'s Code`
                          : "Asset Type"
                      }
                      style={{ width: "100%" }}
                      suffix={
